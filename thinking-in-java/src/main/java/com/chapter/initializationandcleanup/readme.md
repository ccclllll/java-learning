...
```
当初次加载所使用的这个类时，静态成员将被初始化，并且会执行静态初始化代码块,静态初始化代码块会覆盖掉之前的初始化。
...
```
ordinal()可输出常量的申明顺序。枚举类型可用于选择语句(枚举类型和基本类型皆可用于选择语句，引用类型不可)。